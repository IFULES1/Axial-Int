        ...
        <div className="thread-inner">
          {conversation.messages.map((m, i) => (
            m.role === 'user'
              ...
              : <AiMsg key={i} content={m.content} sources={m.sources || []}
...
function AiMsg({ content, sources, streamingSpeed, openCite, isLast }) {
  ...
  // content is array of strings | { cite: id }. The API returns a plain string,
  // so normalize to the array shape the streaming/citation renderer expects.
  const parts = Array.isArray(content) ? content : [content == null ? '' : String(content)];
  // Stream the textual portion gradually
  const fullText = useConvMemo(() => parts.map(p => typeof p === 'string' ? p : `__CITE_${p.cite}__`).join(''), [parts]);
  ...
  // Render shown[0..shown] back into nodes with cite chips
  const nodes = [];
  let cursor = 0;
  for (const part of parts) {
    if (typeof part === 'string') {
      const remaining = shown - cursor;
      ...
      const tag = `__CITE_${part.cite}__`;
      const remaining = shown - cursor;
      if (remaining <= 0) break;
      if (remaining >= tag.length) {
        nodes.push(
          <sup key={`c-${cursor}`} className="cite" onClick={() => openCite(part.cite)}>
            [{part.cite}]
          </sup>
        );
      }
  ...
  return (
    <div className="composer-shell">
      ...
      <div className="composer-tip">
        <Icon name="sparkle" size={13} style={{ color: 'var(--v-soft)' }} />
        <span>
          Astuce — précisez le périmètre (géographie, segment, horizon de temps) pour des
          réponses plus instruites.
        </span>
      </div>
    </div>
  );
...
function renderInline(text, kp) {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    else nodes.push(<sup key={kp + 'c' + i++} style={{ color: 'var(--v-bright)', fontWeight: 600, marginLeft: 1 }}>[{m[2]}]</sup>);
...
function MarkdownView({ text }) {
    ...
    if (bullets.length) {
      blocks.push(
        <ul key={'ul' + k} style={{ margin: '6px 0 6px 20px', display: 'flex', flexDirection: 'column', gap: 5 }}>
          {bullets.map((b, i) => <li key={i} style={{ lineHeight: 1.55 }}>{renderInline(b, 'l' + k + i)}</li>)}
        </ul>,
      );
      bullets = [];
    }
  ...
  lines.forEach((raw, idx) => {
    ...
    if (line.startsWith('### ')) { flush(idx); blocks.push(<h3 key={idx} style={{ fontSize: 15, fontWeight: 700, margin: '14px 0 6px' }}>{renderInline(line.slice(4), 'h' + idx)}</h3>); }
    else if (line.startsWith('## ')) { flush(idx); blocks.push(<h2 key={idx} style={{ fontSize: 17, fontWeight: 700, margin: '18px 0 8px' }}>{renderInline(line.slice(3), 'h' + idx)}</h2>); }
    else if (line.startsWith('# ')) { flush(idx); blocks.push(<h1 key={idx} style={{ fontSize: 20, fontWeight: 800, margin: '8px 0 10px' }}>{renderInline(line.slice(2), 'h' + idx)}</h1>); }
    ...
    else { flush(idx); blocks.push(<p key={idx} style={{ margin: '6px 0', lineHeight: 1.6 }}>{renderInline(line, 'p' + idx)}</p>); }
  });
  flush('end');
...
function App() {
  ...
  const handleSendInActive = async (text) => {
    ...
    try {
      const reply = await axChat(text);
      setConversations((cs) => cs.map((c) => c.id === cid ? {
        ...c, messages: [...c.messages.slice(0, -1), { role: 'assistant', content: reply.content }],
      } : c));
    } catch (e) {
      setConversations((cs) => cs.map((c) => c.id === cid ? {
  ...
  const handleSendNew = async (text) => {
    ...
    const now = (window.AXIAL_LANG === 'en') ? 'now' : 'à l\'instant';
    setConversations((cs) => [{
      id, title, lastUpdated: now,
      messages: [{ role: 'user', content: text }, { role: 'assistant', content: '__PENDING__' }],
    }, ...cs]);
    setActiveId(id);
    try {
      await axNewConversation();  // this analysis gets its own backend conversation
      const reply = await axChat(text);
      setConversations((cs) => cs.map((c) => c.id === id ? {
        ...c, messages: [{ role: 'user', content: text }, { role: 'assistant', content: reply.content }],
      } : c));
    } catch (e) {
      setConversations((cs) => cs.map((c) => c.id === id ? {
