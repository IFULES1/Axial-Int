...
/* sidebar.js — collapsible left nav (rail on desktop), persistence + broadcast,
   modeled on theme.js above. 'open' | 'rail'. */

window.setAxialSidebar = function (mode) {
  if (mode !== 'open' && mode !== 'rail') return;
  document.documentElement.dataset.sidebar = mode;
  window.AXIAL_SIDEBAR = mode;
  try { localStorage.setItem('axial_sidebar', mode); } catch (_) {}
  window.dispatchEvent(new Event('axial:sidebar'));
};

window.useSidebar = function useSidebar() {
  const [mode, setMode] = React.useState(() => window.AXIAL_SIDEBAR || 'open');
  React.useEffect(() => {
    const onChange = () => setMode(window.AXIAL_SIDEBAR || 'open');
    window.addEventListener('axial:sidebar', onChange);
    return () => window.removeEventListener('axial:sidebar', onChange);
  }, []);
  return [mode, window.setAxialSidebar];
};

(function initSidebar() {
  let saved = 'open';
  try { saved = localStorage.getItem('axial_sidebar') || 'open'; } catch (_) {}
  window.AXIAL_SIDEBAR = (saved === 'rail') ? 'rail' : 'open';
...
function Icon({ name, size = 18, stroke = 1.6, ...rest }) {
  ...
  switch (name) {
    ...
    case 'chevrons-left': return (
      <svg {...common}><path d="m11 17-5-5 5-5M18 17l-5-5 5-5" /></svg>
    );
    case 'chevrons-right': return (
      <svg {...common}><path d="m13 17 5-5-5-5M6 17l5-5-5-5" /></svg>
    );
    case 'menu': return (
  ...
  const ToolBtn = ({ id, icon, label }) => (
    <li>
      <button
        ...
        title={label}
        onClick={() => goSubRoute(id)}>
        <Icon name={icon} size={14} /> <span className="label">{label}</span>
      </button>
    </li>
  );
    ...
    <div className="app-shell">
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-foot">
          ...
          <button className="btn btn-ghost" title={t('nav.logout')} style={{ width: '100%' }} onClick={onLogout}>
            <Icon name="logout" size={14} /> <span className="label">{t('nav.logout')}</span>
          </button>
        </div>
      </aside>
...
var { useEffect: useEffectS, useMemo: useMemoS, useRef: useRefS } = React;
...
function SettingsSurface() {
  ...
  const [notif, setNotifState] = React.useState({ findings: true, weekly: true, marketing: false });
...
const TOP_ROUTES = ['landing','auth','onb1','onb2','onb3','onb4','app'];
