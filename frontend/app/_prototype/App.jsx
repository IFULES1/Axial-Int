...
function App() {
  ...
  const handleSendInActive = async (text) => {
    ...
    try {
      ...
      setConversations((cs) => cs.map((c) => c.id === cid ? {
        ...c, messages: [...c.messages.slice(0, -1), { role: 'assistant', content: reply.content, agent: reply.agent }],
      } : c));
    } catch (e) {
      setConversations((cs) => cs.map((c) => c.id === cid ? {
  ...
  const handleSendNew = async (text) => {
    ...
    try {
      ...
      setConversations((cs) => cs.map((c) => c.id === id ? {
        ...c, messages: [{ role: 'user', content: text }, { role: 'assistant', content: reply.content, agent: reply.agent }],
      } : c));
    } catch (e) {
      setConversations((cs) => cs.map((c) => c.id === id ? {
